$(window).load(function() {
	...
	$("#alarm-code-add").validatebox({
		required : true
	});
	$("#alarm-name-add").validatebox({
		required : true
	});
	$("#alarm-code-edit").validatebox({
		required : true
	});
	$("#alarm-name-edit").validatebox({
		required : true
	});
	...
	$("#dg").datagrid({url : "alarm-type!query.do"}).datagrid("load", {});
});
...
var dlg_btn_edit_cfg = {
	buttons : [ {
		...
		handler : function() {
			...
			if (row) {
				$('#fm_edit').form('submit', {
					url : "alarm-type!edit.do?id=" + row.id,
					onSubmit : function() {
						return $(this).form("validate");
					},
					success : function(result) {
