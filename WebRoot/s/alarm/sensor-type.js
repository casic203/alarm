...
var dlg_btn_edit_cfg = {
	buttons : [ {
		...
		handler : function() {  
			...
			if (row) {
				$('#fm_edit').form('submit', {
					url : "sensor-type!edit.do?id=" + row.id,
					onSubmit : function() {
						return $(this).form("validate");
					},
					success : function(result) {
...
var dlg_btn_query_cfg = {
	buttons : [ {
		...
		handler : function() {
			$("#dg").datagrid("load", {   
				isuse : $("#param-isuse-query").combobox("getValue")
			});
