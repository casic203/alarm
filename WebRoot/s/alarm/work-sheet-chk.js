...
function loadPage(){
	...
	$("#dg").datagrid(_dg_cfg).datagrid({url : "work-sheet-chk!query.do"});
}
...
var _dg_cfg = {
	...
	toolbar : [ {
		...
		handler : function() {
			...
			if (row) {
				$.messager.confirm("Confirm", "您确定批准吗?", function(r) {
					if (r) {
						$.post("work-sheet-chk!passWorkSheet.do", {
							id : row.id
						}, function(result) {
							if (result.success) {
								$("#dg").datagrid("reload");
		...
		handler : function() {
			var row = $("#dg").datagrid("getSelected");
			if (row) {
				$.messager.confirm("Confirm", "您确定否決吗?", function(r) {
					if (r) {
						$.post("work-sheet-chk!dropWorkSheet.do", {
							id : row.id
						}, function(result) {
							if (result.success) {
								$("#dg").datagrid("reload");
