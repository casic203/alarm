...
public class WorkSheetChkAction extends BaseAction implements
	...
	public void query() {
		try {
			...
			hqlRow.append("from WorkSheet where sheetStatus='待审'");
			hqlCount.append("select count(*) from WorkSheet where sheetStatus='待审'");
	...
	public void passWorkSheet(){
		try {
			if(model.getId()!=null){
				model=workSheetManager.findUniqueBy("id", model.getId());
				model.setChecker(objUser.getPrincipal().getName());
				model.setSheetStatus("批准");
				workSheetManager.save(model);
				
				SysLog log=new SysLog();
				log.setBusinessName("工单审核操作");
				log.setContent("审核通过："+model.getSheetNo()+"-"+model.getDevice().getDevCode());
				...
				JSONTool.writeMsgResult(false, "请选择要批准的工单！");
			}
			JSONTool.writeMsgResult(true, "批准操作完成！");
		} catch (Exception e) {
			try {
				e.printStackTrace();
				JSONTool.writeMsgResult(true, "批准操作失败！");
	...
	public void dropWorkSheet() {
		try {
			if(model.getId()!=null){
				model=workSheetManager.findUniqueBy("id", model.getId());
				model.setChecker(objUser.getPrincipal().getName());
				model.setSheetStatus("否决");
				workSheetManager.save(model);
				
				SysLog log=new SysLog();
				log.setBusinessName("工单审核操作");
				log.setContent("审核否决："+model.getSheetNo()+"-"+model.getDevice().getDevCode());
				log.setOperationType("add");
				log.setCreateUser(objUser.getPrincipal().getName());
				log.setCreateTime(new Date());
				
			}else {
				JSONTool.writeMsgResult(false, "请选择要否决的工单！");
			}
			JSONTool.writeMsgResult(true, "否决操作完成！");
		} catch (Exception e) {
			try {
				e.printStackTrace();
				JSONTool.writeMsgResult(true, "否决操作失败！");
			} catch (IOException e1) {
				e1.printStackTrace();
			}
		}
